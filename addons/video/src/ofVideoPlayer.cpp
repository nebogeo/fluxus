...
bool createMovieFromPath(char * path, Movie &movie)
{
	...
	FSSpec	theFSSpec;
		...
		if (result != noErr) {
			cerr << "NativePathNameToFSSpec failed " <<  result << endl;
			cerr << "Error loading movie" << endl;
			return false;
		}
		...
		if (result) {
			cerr << "FSPathMakeRef failed " << result << endl;
			cerr << "Error loading movie" << endl;
			return false;
		}
		result = FSGetCatalogInfo(&fsref, kFSCatInfoNone, NULL, NULL, &theFSSpec, NULL);
		if (result) {
			cerr << "FSGetCatalogInfo failed " << result << endl;
			cerr << "Error loading movie" << endl;
			return false;
		}
	#endif
	...
	if (result == noErr) {
		...
		if (result == noErr){
			...
			cerr << "NewMovieFromFile failed " << result << endl;
			return false;
		}
	} else {
		cerr << "OpenMovieFile failed " << result << endl;
		return false;
	}
...
bool createMovieFromURL(string urlIn,  Movie &movie){
	...
	if ( ( err = MemError()) != noErr){ cerr << "createMovieFromURL: error creating url handle" << endl; return false;}
	...
	if(err != noErr){
		cerr << "createMovieFromURL: error loading url" << endl;
		return false;
	}else{
		return true;
	}
...
ofVideoPlayer::ofVideoPlayer (){
	...
	//--------------------------------------------------------------
    #ifndef __linux__	// !linux = quicktime...
    //--------------------------------------------------------------
	moviePtr						= NULL;
	allocated						= false;
...
ofTexture & ofVideoPlayer::getTextureReference(){
	if(!tex.bAllocated() ){
		cerr << "ofVideoPlayer - getTextureReference - texture is not allocated" << endl;
	}
	return tex;
}
*/
...
void ofVideoPlayer::idleMovie(){
		...
		//--------------------------------------------------------------
		if (bLoaded == true){
			#if defined(WIN32) || defined(QT_USE_MOVIETASK)
	...
	//--------------------------------------
		...
		if(gst_element_set_state(GST_ELEMENT(gstPipeline), GST_STATE_PAUSED) ==
		   GST_STATE_CHANGE_FAILURE) {
			cerr << "GStreamer: unable to set pipeline to paused" << endl;
			gst_object_unref(gstPipeline);
			return false;
		}
...
void ofVideoPlayer::setPosition(float pct){
	...
	if(speed>0){
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed,format,
				...
				GST_SEEK_TYPE_SET,
				-1)) {
		cerr << "GStreamer: unable to change speed" << endl;
		}
	}else{
		if(!gst_element_seek(GST_ELEMENT(gstPipeline),speed,format,
				flags,
...
float ofVideoPlayer::getPosition(){
	...
	//--------------------------------------
		...
		if(!gst_element_query_position(GST_ELEMENT(gstPipeline),&format,&pos))
			cerr << "GStreamer: cannot query position" << endl;
...
bool ofVideoPlayer::allocate(){
	...
	if(!gst_element_query_duration(gstPipeline,&format,&durationNanos))
		cerr << "GStreamer: cannot query time duration" << endl;
...
void ofVideoPlayer::gstHandleMessage()
{
	...
	while(gst_bus_have_pending(bus)) {
		...
		// cerr << "GStreamer: Got " << GST_MESSAGE_TYPE_NAME(msg) << " message" << endl;
		...
		switch (GST_MESSAGE_TYPE (msg)) {
			...
			case GST_MESSAGE_DURATION:{
				GstFormat format=GST_FORMAT_TIME;
				if(!gst_element_query_duration(gstPipeline,&format,&durationNanos))
					cerr << "GStreamer: cannot query duration" << endl;
			}break;
			...
			case GST_MESSAGE_STATE_CHANGED:
				...
				// cerr << "GStreamer: state changed from " << oldstate << " to " << newstate << "(" << pendstate << ")" << endl;
			break;
			...
			case GST_MESSAGE_ERROR: {
				...
				cerr << "GStreamer Plugin: Embedded video playback halted; module " <<
					gst_element_get_name(GST_MESSAGE_SRC (msg)) << " reported: " << err->message << endl;
			...
			case GST_MESSAGE_EOS:
				// cerr << "GStreamer: end of the stream." << endl;
				bIsMovieDone = true;
				...
				switch(loopMode){
					...
					case OF_LOOP_NORMAL:{
						...
						if(!gst_element_seek(GST_ELEMENT(gstPipeline),
											...
											GST_SEEK_TYPE_SET,
											durationNanos)) {
							cerr << "GStreamer: unable to seek" << endl;
						}
					}break;
			...
			default:
				//cerr << "GStreamer: unhandled message" << endl;
			break;
		}
