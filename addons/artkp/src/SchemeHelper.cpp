...
void SchemeHelper::DoublesFromScheme(Scheme_Object *src, double *dst, unsigned int size)
{
	MZ_GC_DECL_REG(1);
	MZ_GC_VAR_IN_REG(0, src);
	MZ_GC_REG();
...
Scheme_Object *SchemeHelper::DoublesToScheme(double *src, unsigned int size)
{
	Scheme_Object *ret=NULL;
	Scheme_Object *tmp=NULL;
	MZ_GC_DECL_REG(2);
	...
	//if(argc==(int)format.size())
	{
		for (unsigned int n=0; n<format.size(); n++)
		{
			switch(format[n])
			{
				...
				case 'I':
					if (!(SCHEME_CPTRP(argv[n]) && (StringFromScheme(SCHEME_CPTR_TYPE(argv[n])) == "imgptr")))
					{
						MZ_GC_UNREG();
						scheme_wrong_type(funcname.c_str(), "imgptr", n, argc, argv);
					}
				break;
